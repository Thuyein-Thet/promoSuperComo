...
**Architecture:** A single Next.js (App Router) project. `/api/cron/sync-flyers` is a serverless route triggered on a schedule by Vercel Cron; it fetches Tokubai's pages directly via `fetch()` and parses the HTML with `cheerio` to discover stores and scrape flyers, diffs against Postgres, and syncs Blob storage. The `/` page renders a client-side Leaflet map (fed by a `/api/stores` route) with clustering, search, and a responsive popup/panel for viewing flyers.

**Tech Stack:** Next.js 16 (App Router, TypeScript), `cheerio` (^1.2.0) for HTML parsing, `pg` (^8.22.0) against Vercel Postgres/Neon's standard connection string, `@vercel/blob` (^2.4.1), `leaflet` (^1.9.4) + `react-leaflet` (^5.0.0) + `leaflet.markercluster` (^1.5.3), Vitest (^4.1.9) for unit tests.
...
**Note on `cheerio` vs the Firecrawl SDK:** the plan originally specified the `firecrawl` SDK (a managed headless-browser scraping API) for all of Tokubai. Real usage showed one full 82-store sync consumed roughly half the user's monthly Firecrawl credit allowance, making any regular cron schedule unsustainable on a free plan. Tokubai's pages turned out to be plain server-rendered HTML (confirmed via direct `curl`), so Firecrawl's JS-rendering capability was never actually needed for this site — the scraper was rewritten to use native `fetch()` + `cheerio`, removing the `firecrawl` dependency and all per-request cost entirely. See the Self-Review Notes for full details of this rewrite.
...
Paste a generated random secret when prompted:
...
cp .env.example .env.local  # fill in POSTGRES_URL, CRON_SECRET
npm install
npm run dev
\`\`\`
...
- **Major amendment (post rate-limit fix, replacing Firecrawl entirely):** one full sync consumed ~500 of the user's 1,000 monthly Firecrawl credits, making a daily (or even weekly) cron unsustainable on a free plan. Investigation found that Tokubai's pages are plain server-rendered HTML — confirmed via direct `curl` with a browser `User-Agent` header, no JavaScript execution needed — so Firecrawl's headless-browser rendering was unnecessary for this site. Replaced the entire scraping layer with native `fetch()` + `cheerio` HTML parsing, removing the `firecrawl` npm dependency completely:
  - `src/lib/tokubai.ts`'s three parsers (`parseStoreList`, `parseStoreDetail`, `parseFlyerImages`) now take raw HTML instead of Firecrawl's markdown, selecting via CSS classes (`div[class^='shop_leaflet_index_card ']` for store cards, `a.shop_name` and `.address a[href*='maps']` for store detail, and a regex over the embedded `data-view-state` JSON blob on leaflet pages for flyer images) instead of fuzzy text/regex matching over markdown. This is also strictly more precise: the old regex-based `STORE_LINK_RE` had produced one false-positive match (the page header's logo link); the CSS-class selector cannot match unrelated elements.
  - `src/lib/sync.ts`'s `FirecrawlClient` interface (`scrape(url): Promise<{markdown}>`) was replaced with a simpler `HttpClient` (`fetchText(url): Promise<string>`). The `rateLimited()` wrapper and `requestsPerMinute` option were removed entirely — there is no metered quota to protect against anymore, only a default `concurrency: 5` for politeness toward Tokubai's servers.
